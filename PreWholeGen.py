...
def DNA_complement2(sequence):  #碱基互补配对
    trantab = str.maketrans('ACGTNRacgt', 'TGCACCtgca')     # trantab = str.maketrans(intab, outtab)   # 制作翻译表
...
def predict(model,threshold=0.9999):
    ...
    preACC=model.predict(x,verbose=0)
    ...
    plt.scatter(a[0],a[1],s=400,c='green',alpha=0.3, cmap=None)
